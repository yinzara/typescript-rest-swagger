...
var SpecGenerator = /** @class */ (function () {
    ...
    SpecGenerator.prototype.buildDefinitions = function () {
        var _this = this;
        var definitions = {};
        var ignoreTypes = this.config.ignoreTypes || [];
        var includeTypes = this.config.includeTypes;
        Object.keys(this.metadata.referenceTypes)
            .filter(function (typeName) { return includeTypes === undefined || includeTypes.some(function (type) { return mm(typeName, type); }); })
            .filter(function (typeName) { return !ignoreTypes.some(function (type) { return mm(typeName, type); }); })
            ...
            if (referenceType.typeAlias) {
                if (['oneOf', 'anyOf', 'allOf'].includes(referenceType.typeAlias.typeName)) {
                    var types = referenceType.typeAlias.types.map(_this.getSwaggerType.bind(_this));
                    definitions[referenceType.typeName] = __assign(__assign(__assign({ description: referenceType.description }, (referenceType.typeAlias.typeName === 'allOf' ? { allOf: types } : {})), (referenceType.typeAlias.typeName === 'anyOf' ? { anyOf: types } : {})), (referenceType.typeAlias.typeName === 'oneOf' ? { oneOf: types } : {}));
                }
                else {
                    definitions[referenceType.typeName] = __assign({ description: referenceType.description }, _this.getSwaggerType(referenceType.typeAlias));
                }
            }
            else {
    ...
    SpecGenerator.prototype.buildProperties = function (properties) {
        ...
        var ignoreProperties = this.config.ignoreProperties || [];
        properties

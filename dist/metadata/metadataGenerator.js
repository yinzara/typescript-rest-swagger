...
var MetadataGenerator = /** @class */ (function () {
    ...
    MetadataGenerator.prototype.getClassDeclaration = function (className, nodes) {
        var _this = this;
        if (nodes === void 0) { nodes = this.nodes; }
        var moduleIndex = className.indexOf('.');
        var found;
        if (moduleIndex > 0) {
            var moduleName_1 = className.substring(0, moduleIndex);
            found = nodes
                .map(function (node) { return node; })
                .filter(function (node) { return node.kind === ts.SyntaxKind.ModuleDeclaration && node.name && node.name.text === moduleName_1; })
                .map(function (node) { return 'statements' in node.body && _this.getClassDeclaration(className.substring(moduleIndex + 1), node.body.statements); })
                .filter(function (node) { return node; });
        }
        else {
            found = nodes
                .map(function (node) { return node; })
                .filter(function (node) { return node.kind === ts.SyntaxKind.ClassDeclaration && node.name && node.name.text === className; });
        }
    ...
    MetadataGenerator.prototype.getInterfaceDeclaration = function (className, nodes) {
        var _this = this;
        if (nodes === void 0) { nodes = this.nodes; }
        var moduleIndex = className.indexOf('.');
        var found;
        if (moduleIndex > 0) {
            var moduleName_2 = className.substring(0, moduleIndex);
            found = nodes
                .map(function (node) { return node; })
                .filter(function (node) { return node.kind === ts.SyntaxKind.ModuleDeclaration && node.name && node.name.text === moduleName_2; })
                .map(function (node) { return 'statements' in node.body && _this.getInterfaceDeclaration(className.substring(moduleIndex + 1), node.body.statements); })
                .filter(function (node) { return node; });
        }
        else {
            found = nodes
                .map(function (node) { return node; })
                .filter(function (node) { return node.kind === ts.SyntaxKind.InterfaceDeclaration && node.name && node.name.text === className; });
        }
        if (found && found.length) {
            return found[0];
        }
